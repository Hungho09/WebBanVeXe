...
export class BookingManagement implements OnInit {
  ...
  confirmPayment(id: string) {
    if (confirm('Xác nhận khách hàng đã thanh toán vé này? Trạng thái sẽ chuyển sang "Đã thanh toán" và ghi nhận vào doanh thu.')) {
      const adminUserId = this.authService.getUser().id;
      if (!adminUserId) {
        this.toast.showError('Không tìm thấy thông tin admin đăng nhập');
        return;
      }
      this.bookingService.confirmPayment(id, adminUserId).subscribe({
        next: (res) => {
          this.toast.showSuccess(res.message || 'Đã xác nhận thanh toán thành công');
          this.loadBookings();
        },
        error: (err) => this.toast.showError(err.error?.message || 'Lỗi khi xác nhận thanh toán')
      });
    }
  }
}
